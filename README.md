一款基于 Swin Transformer V2 的图形化图片分类工具，支持自定义训练参数、自动保存配置、实时进度显示，可识别特定格式文件夹的图片分类设计，自动跳过含关键词的文件夹，兼容 CPU/GPU 运行。
✨ 核心特性
...
- 🖼️ 智能图片处理：自动遍历类似名称文件夹下所有子文件夹，跳过含某些关键词的文件夹，支持 JPG/JPEG/PNG/BMP 格式。
...
分类模式
加载训练好的模型，批量分类文件夹下的所有图片，按分类结果和文件夹ID组织输出路径，自动跳过相关文件夹。
📥 环境依赖
